...
invoices.totalDue = function () {
  return this.
         unpaid.
         reduce((acc, elm) => acc + elm.amount, 0);
}
...
invoices.payInvoice = function (name) {
  var newUnpaid = [];
  var paid = this.paid; // `this` inside forEach points to the global object

  this.unpaid.forEach(function (elm) {
    console.log(this);
    elm.name === name ? paid.push(elm) : newUnpaid.push(elm);
  });
